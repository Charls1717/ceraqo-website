...
#   bridge/out/clip4k_N.mp4   AI-upscaled 4K master (preferred when present
...
rm -rf public/frames
mkdir -p public/frames/desktop public/frames/hidpi public/frames/mobile public/frames/stills
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
...
for i in 0 1 2 3 4; do
  ...
  for t in "${TIERS[@]}"; do
    ...
    ffmpeg -hide_banner -loglevel error -i "$clip" \
      -vf "select='not(mod(n\,$STEP))',scale=${TIER_W[$t]}:-2:flags=lanczos" -vsync vfr \
