...
# Container/CI runs have no system ffmpeg — the npm-installed binaries
# (ffmpeg-static / ffprobe-static) are passed via env by the caller.
...
def stats(seg):
    ...
    out = {
        ...
        "episodes": [
            {"atS": round(s / fps, 2), "lenS": round(l / fps, 2)} for s, l in episodes
        ],
        "medianGapS": None,
    }
    if len(episodes) >= 2:
